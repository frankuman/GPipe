...
class Gerrit:
    ...
    def requestAPICall(
        PLATFORM, DATE_1, DATE_2, SET_TIME_1, SET_TIME_2, root, crawl=None
    ):
        """
        does API stuff
        """
        if PLATFORM != "https://review.opendev.org":
            error = Gerrit.REST_call_google(
            ...
            while True:
                res, error = response.changes(
                    PLATFORM, date_string, root, limit=5000, start=start
                )
                if not res:
                    break
